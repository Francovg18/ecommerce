...
class ContactCreateView(APIView):
    ...
    def post(self, request, format=None):
        ...
        if not all([name, email, subject, message, phone, city]):
        ...
        try:
            ...
            Contact.objects.create(
                name=name, email=email, subject=subject, phone=phone, message=message, city=city
            )
            ...
            tag_response = requests.post(
                tag_url, json=tag_payload, headers=headers)
            ...
            list_payload = {'contactList': {'list': '2',
                                            'contact': contact_id, 'status': '1'}}
            list_response = requests.post(
                list_url, json=list_payload, headers=headers)
...
class EbookSubscriptionView(APIView):
    ...
    def post(self, request, format=None):
        ...
        try:
            ...
            list_url = f"{activecampaign_url}/api/3/contactLists"
            list_payload = {'contactList': {'list': '4',
                                            'contact': contact_id, 'status': '1'}}
            list_response = requests.post(
                list_url, json=list_payload, headers=headers)
