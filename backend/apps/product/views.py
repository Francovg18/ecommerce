...
class ListSearchView(APIView):
    ...
    def post(self, request, format=None):
        ...
        if len(search) == 0:
            ...
            # filtramos con criterio usando Q
            search_results = Product.objects.filter(
                Q(description__icontains=search) | Q(name__icontains=search)
            )

        if category_id == 0:
            search_results = ProductSerializer(
...
class ListRelatedView(APIView):
    ...
    def get(self, request, productId, format=None):
        ...
        if Product.objects.filter(category=category).exists():
            ...
            if len(related_products.data) > 5:
                return Response(
                    {'related_products': related_products.data[:5]},
                    status=status.HTTP_200_OK)
...
class ListBySearchView(APIView):
    ...
    def post(self, request, format=None):
        ...
        if category_id == 0:
            ...
            category = Category.objects.get(id=category_id)
            if category.parent:
                # Si la categoria tiene padrem filtrar solo por la categoria y no el padre
        ...
        product_results = ProductSerializer(
            product_results, many=True, context={'request': request})
