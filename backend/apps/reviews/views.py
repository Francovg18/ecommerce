...
class UpdateProductReviewView(APIView):
    def put(self, request, productId, format=None):
        ...
        # Validar la calificación (rating)
        try:
            rating = float(data['rating'])
            ...
            return Response(
                {'error': 'Rating must be a decimal value between 1 and 5'},
                status=status.HTTP_400_BAD_REQUEST
            )
        ...
        try:
            ...
            return Response(
                {'error': 'Must pass a non-empty comment'},
                status=status.HTTP_400_BAD_REQUEST
            )

        # Obtener el producto y la reseña
        ...
        try:
            ...
            # Log error
            return Response(
                {'error': f'Something went wrong when updating review: {str(e)}'},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
