...
import {
    ...
    GET_PRODUCTS_FOR_SHOWCASE_SUCCESS,
    GET_PRODUCTS_FOR_SHOWCASE_FAIL
...
export const get_products_by_sold = () => async dispatch => {
    ...
    try {
        const res = await axios.get(`${process.env.REACT_APP_API_URL}/api/product/get-products?sortBy=sold&order=desc&limit=5`, config);
...
export const get_filtered_products = (category_id, price_range, sort_by, order) => async dispatch => {
    ...
    try {
        ...
        console.error("⚠️ Error en get_filtered_products:", err.response ? err.response.data : err);
        dispatch({
            type: FILTER_PRODUCTS_FAIL
        });
    }
...
export const get_products_for_showcase = () => async dispatch => {
    const config = {
        headers: {
            'Accept': 'application/json'
        }
    ...
    try {
        const res = await axios.get(`${process.env.REACT_APP_API_URL}/api/product/get-products?sortBy=date_created&order=desc&limit=10`, config);
    
        if (res.status === 200) {
            dispatch({
                type: GET_PRODUCTS_FOR_SHOWCASE_SUCCESS,
                payload: res.data
            });
        } else {
            dispatch({
                type: GET_PRODUCTS_FOR_SHOWCASE_FAIL
            });
        }
    } catch(err) {
        dispatch({
